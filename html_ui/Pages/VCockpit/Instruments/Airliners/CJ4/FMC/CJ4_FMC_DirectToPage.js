class CJ4_FMC_DirectToPage {
    static ShowPage(fmc, directWaypoint, wptsListIndex = 0) {
        ...
        let i = 0;
        while (i < fmc.flightPlanManager.getWaypointsCount() && i + wptsListIndex < fmc.flightPlanManager.getWaypointsCount() && i < iMax) {
            let waypoint = fmc.flightPlanManager.getWaypoint(i + wptsListIndex);
            if (waypoint) {
